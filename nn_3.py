...
def read_data():
	...
	#input features
	#Feature Selection
	#Selecting only the Timber Average Values
	train_input= df1.iloc[:,1:21].values
	#print(X)
	#normalization
	...
	#Feature Selection
	#Selecting only the Timber Average Values
	dev_input= df2.iloc[:,1:21].values
	#print(X)
	#normalization
	...
	#Feature Selection
	#Selecting only the Timber Average Values
	test_input= df3.iloc[:,0:20].values
	#print(X)
	#normalization
...
def train(FCL1,A1,D12,FCL2,A2,D23,FCL3,A3,lamda,Loss_Fn,optimizer,max_epochs,batch_size,train_input,train_target):
	...
	for epoch in range(1001):
		...
		#early stopping criteria with patience of 50 epochs
...
def main():
	...
	FCL1=Net(20,64,lamda)
	A1=ReLU()

	D12=Dropout(0)

	FCL2=Net(64,32,lamda)
	A2=ReLU()
	...
	df.to_csv(r'Regression/Result_FS.csv')
