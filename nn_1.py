...
# training on dev set with rmse loss
def train_dev(FCL1,A1,D12,FCL2,A2,D23,FCL3,A3,lamda,Loss_Fn,optimizer,max_epochs,batch_size,dev_input,dev_target):
	...
	precision=np.std(dev_target)

	for epoch in range(1001):

		FCL1.forward(dev_input)
		A1.forward(FCL1.output)
		...
		data_loss=Loss_Fn.calculate(A3.output,dev_target)
		loss_rmse=Loss_RMSE()
		rmse_loss=loss_rmse.calculate(A3.output,dev_target)
		reg_loss=Loss_Fn.reg_loss(FCL1)+Loss_Fn.reg_loss(FCL2)+Loss_Fn.reg_loss(FCL3)
		...
		accuracy=np.mean(np.absolute(predictions-dev_target)<precision)
		
		print(	f'epoch: {epoch},' + 
				f'accuracy: {accuracy:.3f},' + 
				f'loss_rmse:{rmse_loss:.3f}, '+
				f'loss:{loss:.3f}, '+
				f'data_loss:{data_loss:.3f}, '+
				f'regularization_loss:{reg_loss:.3f},' + 
				f'lr:{optimizer.current_lr}')
		...
		Loss_Fn.backward(A3.output,dev_target)
